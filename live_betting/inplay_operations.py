...
def save_first_set_odds(odds: tuple, bookmaker_id: int, bookmaker_matchid: str):
    query = "INSERT INTO odds (bookmaker_id, match_bookmaker_id, odds_type, match_part, odd1, odd2) \
                VALUES (%s, %s, %s, %s, %s, %s)"
    params = [bookmaker_id, bookmaker_matchid, 'home_away', 'set1', odds[0], odds[1]]
    ...
    if type(db_returned) == IntegrityError:
        if 'duplicate key value violates unique constraint' in db_returned.args[0]:
            ...
            params = [odds[0], odds[1], bookmaker_id, bookmaker_matchid, 'home_away', 'set1']
