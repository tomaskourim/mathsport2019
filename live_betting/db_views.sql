...
WHERE name IN ('ATP Australian Open', 'ATP US Open', 'ATP French Open', 'ATP Wimbledon') AND
    utc_time_recorded > '2020-09-28 06:00:00.000000'
...
WHERE start_time_utc > '2020-09-28 06:00:00.000000' AND
...
WHERE start_time_utc >= '2020-09-27 06:00:00.000000'
...
FROM (
    ...
    ON odds.bookmaker_id = mb.bookmaker_id AND odds.match_bookmaker_id = mb.match_bookmaker_id
             JOIN matches m ON mb.match_id = m.id
             JOIN tournament t ON m.tournament_id = t.id) AS r
WHERE start_time_utc >= '2020-09-28 06:00:00.000000'
...
WHERE match_bookmaker_id IN (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM match_course
            WHERE utc_time_recorded > '2020-09-28 06:00:00.000000'
...
WHERE mc.start_time_utc > '2020-09-28 06:00:00.000000' AND o.id ISNULL;
...
WHERE match_bookmaker_id IN (
    ...
    WHERE start_time_utc > '2020-09-28 06:00:00.000000' AND mc.utc_time_recorded ISNULL EXCEPT
    SELECT match_bookmaker_id
    FROM inplay)
ORDER BY match_bookmaker_id;
...
FROM bet
WHERE utc_time_recorded >= '2020-09-28 06:00:00.000000' AND result_corrected NOTNULL;
