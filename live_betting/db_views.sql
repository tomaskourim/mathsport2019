...
WHERE (name LIKE 'ATP%' or name LIKE 'WTA%') AND utc_time_recorded > '2021-02-06 20:00:00.000000'
...
FROM (SELECT *
      FROM matches
      WHERE start_time_utc > '2021-02-07 15:00:00.000000' AND
          start_time_utc < '2021-02-07 13:00:00.000000') AS matches
         JOIN matches_bookmaker ON matches.id = match_id
         JOIN
tournament ON matches.tournament_id = tournament.id
WHERE (name LIKE 'ATP%' or name LIKE 'WTA%') AND type = 'singles'
    EXCEPT
SELECT match_bookmaker_id
FROM inplay;
...
WHERE start_time_utc > '2021-02-06 20:00:00.000000' AND (name LIKE '%ATP%' OR name LIKE '%WTA%')
ORDER BY start_time_utc;
...
WHERE start_time_utc >= '2021-02-06 20:00:00.000000'
...
WHERE match_bookmaker_id IN (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM match_course
            WHERE utc_time_recorded > '2021-02-06 20:00:00.000000'
...
WHERE mc.start_time_utc > '2021-02-06 20:00:00.000000' AND o.id ISNULL;
...
WHERE match_bookmaker_id IN (
    ...
    WHERE start_time_utc > '2021-02-06 20:00:00.000000' AND mc.utc_time_recorded ISNULL EXCEPT
    SELECT match_bookmaker_id
    FROM inplay)
ORDER BY match_bookmaker_id;
...
FROM bet
WHERE utc_time_recorded >= '2021-02-06 20:00:00.000000' AND result_corrected NOTNULL;
