...
FROM (SELECT *
      FROM matches
      WHERE start_time_utc > '2020-08-31 13:00:00.000000' AND
          start_time_utc < '2020-08-31 15:00:00.000000') AS matches
...
WHERE start_time_utc > '2020-08-31 13:00:00.000000' AND (name LIKE '%ATP%' OR name LIKE '%WTA%')
...
WHERE start_time_utc >= '2020-08-01 07:00:00.000000'
...
FROM (
    ...
    ON odds.bookmaker_id = mb.bookmaker_id AND odds.match_bookmaker_id = mb.match_bookmaker_id
             JOIN matches m ON mb.match_id = m.id
             JOIN tournament t ON m.tournament_id = t.id) AS r
WHERE start_time_utc >= '2020-08-31 07:00:00.000000'
...
WHERE match_bookmaker_id IN (
    ...
    FROM (
        ...
        FROM (
            ...
            FROM match_course
            WHERE utc_time_recorded > '2020-08-31 07:00:00.000000'
...
WHERE mc.start_time_utc > '2020-08-31 07:00:00.000000' AND o.id ISNULL;
...
WHERE match_bookmaker_id IN (
    ...
    WHERE start_time_utc > '2020-08-31 07:00:00.000000' AND mc.utc_time_recorded ISNULL EXCEPT
    SELECT match_bookmaker_id
    FROM inplay)
ORDER BY match_bookmaker_id;
...
FROM bet
WHERE utc_time_recorded >= '2020-08-31 07:00:00.000000' AND result_corrected NOTNULL;
