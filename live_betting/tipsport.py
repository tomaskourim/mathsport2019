        ...
        while True:
            try:
                ...
                if home_probability > 1 / set_odds[0]:
                    logging.info("Would be placing bet on home, match {bookmaker_matchid}, set{set_number} "
                                 "with odds {set_odds[0]} and computed prob. {home_probability}")
                ...
                if (1 - home_probability) > 1 / set_odds[1]:
                    logging.info("Would be placing bet on away, match {bookmaker_matchid}, set{set_number} "
                                "with odds {set_odds[0]} and computed prob. {home_probability}")
